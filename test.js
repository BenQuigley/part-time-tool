...
function action(requests) {
  ...
  for (i in requests) {
    ...
    if (req['decision'] && req['isEmailSent'] == '') {
      ...
      sheet.getRange(2+parseInt(i, 10), 17).setValue(req['decision']+' - sent to '+req['username']+' on ' +
                                                            currentDate + ' at ' + currentTime);
    }
    else if (req['decision'] == '' && req['reminderEmailSentTo'] == '') {
      sheet.getRange(2 + parseInt(i, 10), 18).setValue(request['owner']+
                                                              ' on '+currentDate+
                                                              ' at '+currentTime);
